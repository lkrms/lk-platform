...
function lk_hosting_php_get_default_version() { (
    . /etc/lsb-release &&
        ...
        20.04)
            ...
            ;;
        *)
            lk_warn "Ubuntu release not supported: $DISTRIB_RELEASE"
            false
            ;;
        esac
); }
...
END                     { print max + 1 }' $SAME_ROOT)} || return
    ...
    for ALIAS in $SITE_ALIASES; do
        ...
        SAME_DOMAIN=$(awk -v "d=${1,,}" -v "a=,$ALIAS," \
            '$1 != d && index(","$1",www."$1","$10",", a) {print $1}' \
            "$_SITE_LIST") || return
        [ -z "$SAME_DOMAIN" ] ||
...
# lk_hosting_site_configure DOMAIN [SITE_ROOT [ALIAS...]]
function lk_hosting_site_configure() { (
    LK_SUDO=1
    LK_VERBOSE=${LK_VERBOSE-1}
    [ $# -ge 1 ] || lk_usage "\
Usage: $(lk_myself -f) DOMAIN [SITE_ROOT [ALIAS...]]" || return
