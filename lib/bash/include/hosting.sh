...
function lk_hosting_certbot_maybe_install() {
    ...
    lk_tty_print "Checking TLS certificates for" \
        "$(lk_plural -v SITE_DOMAINS domains)"
    lk_arr_from CERT_DOMAINS lk_certbot_list_all_domains &&
        lk_arr_from NO_CERT_DOMAINS lk_arr_diff SITE_DOMAINS CERT_DOMAINS || return
    [[ -n ${NO_CERT_DOMAINS+1} ]] || {
        lk_tty_success "No certificates to acquire"
        return
    }
    # 1. Convert the system's public IP address(es) to a regular expression
    REGEX=$(lk_system_get_public_ips | lk_ere_implode_input -e) || return
    [[ -n ${REGEX:+1} ]] || lk_warn "public IP address not found" || return 0
    # 2. Check for NO_CERT_DOMAINS entries that resolve to the system
    ...
    lk_hosting_list_sites -e |
        ...
        [[ -s $REQUESTS ]] ||
        lk_warn "error building certificate request site list" || return
