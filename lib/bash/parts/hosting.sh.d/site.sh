...
function lk_hosting_site_list() { (
    ...
    for FILE in "$LK_BASE/etc/sites"/*.conf; do
        ...
        lk_is_true _SITE_IS_CHILD && _SITE_IS_CHILD=Y || _SITE_IS_CHILD=N
        lk_elevate gnu_stat -L \
            --printf "$DOMAIN\\t$SITE_ENABLE\\t%n\\t%i\\t%U\\t$_SITE_IS_CHILD\\t$SITE_PHP_FPM_POOL\\t$SITE_PHP_FPM_USER\\t$SITE_ORDER\\t$SITE_ALIASES\\n" \
            "$SITE_ROOT" || return
