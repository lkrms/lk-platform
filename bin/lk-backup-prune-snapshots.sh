...
function first_snapshot_on_date() {
    local i
    for i in $(seq $((${#SNAPSHOTS_CLEAN[@]} - 1)) -1 0); do
        [[ ${SNAPSHOTS_CLEAN[$i]} == $1* ]] || continue
        echo "${SNAPSHOTS_CLEAN[$i]}"
        return
    done
    false
}
...

{
    ...
    lk_mapfile -z <(find "$BACKUP_ROOT/snapshot" -mindepth 1 -maxdepth 1 \
        -type d -printf '%f\0' | sort -z) SOURCE_NAMES
    for SOURCE_NAME in ${SOURCE_NAMES[@]+"${SOURCE_NAMES[@]}"}; do
        ...
        # Keep one snapshot per week indefinitely
        ...
        lk_mapfile -z <(comm -z -23 \
            <(printf '%s\0' "${SNAPSHOTS_CLEAN[@]}" | sort -z) \
            <(printf '%s\0' "${SNAPSHOTS_KEEP[@]}" | sort -z)) \
            SNAPSHOTS_PRUNE
