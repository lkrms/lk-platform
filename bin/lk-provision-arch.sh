...
if ! lk_is_bootstrap; then
    [ -n "${LK_TIMEZONE-}" ] || ! _TZ=$(lk_system_timezone) ||
        set -- --set LK_TIMEZONE "$_TZ" "$@"
    [ -n "${LK_HOSTNAME-}" ] || ! _HN=$(lk_hostname) ||
        set -- --set LK_HOSTNAME "$_HN" "$@"
    [ -n "${LK_LOCALES+1}" ] ||
        set -- --set LK_LOCALES "en_AU.UTF-8 en_GB.UTF-8" "$@"
    [ -n "${LK_LANGUAGE+1}" ] ||
        set -- --set LK_LANGUAGE "en_AU:en_GB:en" "$@"
fi
...

{
    ...
    FILE=/etc/udev/rules.d/10-lk-local.rules
    ...
    for _FILE in \
        ...
        FILE=${_FILE##*/}
        FILE=/etc/udev/rules.d/85-lk-${FILE/.template./.}
        lk_install -m 00644 "$FILE"
    ...
    if [ -n "${LK_HOSTNAME-}" ]; then
        lk_tty_print "Checking system hostname"
        FILE=/etc/hostname
        lk_install -m 00644 "$FILE"
        lk_file_replace "$FILE" "$LK_HOSTNAME"
...
::1 localhost
$IP_ADDRESS \
${LK_FQDN:-$LK_HOSTNAME.localdomain} \
$LK_HOSTNAME" &&
            awk \
                -v "BLOCK=$HOSTS" \
        ...
        lk_tty_error \
            "Cannot check hostname or /etc/hosts: LK_HOSTNAME is not set"
    fi
    ...
    FILE=/etc/profile.d/Z90-lk-umask.sh
    lk_install -m 00644 "$FILE"
    ...
    for FILE in default.conf \
        ...
        TARGET=/etc/sysctl.d/90-${FILE/default/lk-default}
    ...
    if lk_pac_installed tlp; then
        ...
        unset LK_FILE_REPLACE_NO_CHANGE
        FILE=/etc/tlp.d/90-lk-default.conf
        lk_install -m 00644 "$FILE"
        ...
        file_delete "/etc/tlp.d/90-lk-defaults.conf"
        SERVICE_ENABLE+=(
...
EOF
        ...
        if lk_is_desktop; then
            FILE=/etc/httpd/conf/extra/lk-default-dev-arch.conf
            lk_install -m 00644 "$FILE"
            lk_file_replace \
