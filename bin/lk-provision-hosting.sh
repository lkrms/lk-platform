...
lk_include debian git hosting linux provision validate
...
$IPV6_ADDRESS $HOST_NAMES}" &&
    ...
    if ! lk_is_bootstrap; then
        ...
        ! lk_files_exist "$FILE"{.orig,} ||
            diff -q "$FILE"{.orig,} >/dev/null ||
            {
                lk_console_detail "Restoring original:" "$FILE"
                lk_file_backup "$FILE" &&
                    mv -fv "$FILE"{.orig,} &&
                    LK_FILE_REPLACE_NO_CHANGE=0
            }
    ...
    [ "$STATUS" = P ] || {
        ...
        lk_console_warning "System will be inaccessible if SSH is down"
    }
    ...
    lk_run_detail modprobe nf_conntrack_ftp nf_nat_ftp ||
        lk_die "error loading kernel modules"
    FILE=/etc/modules-load.d/${LK_PATH_PREFIX}nf_conntrack.conf
    lk_install -m 00644 "$FILE"
    lk_file_replace \
        -f "$LK_BASE/share/modules-load.d/nf_conntrack.conf" \
        "$FILE"
    ...
    if lk_dpkg_installed apt-listchanges apticron; then
        ...
        LK_CONF_OPTION_FILE=/etc/apt/listchanges.conf
    ...
    [ -z "${_LK_HOST_KEYS-}" ] || {
        install -d -m 00700 "$DIR/.ssh" &&
            lk_install -m 00600 "$DIR/.ssh/authorized_keys" &&
            lk_file_replace "$DIR/.ssh/authorized_keys" "$_LK_HOST_KEYS"
    }
